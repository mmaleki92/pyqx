...
class NewFileDialog(QtWidgets.QDialog):
    ...
    def __init__(self, context, Parent=None):
        ...
        dimensionGroup = QtWidgets.QGroupBox(
            self.context.getText("dialog_new_image", "dimension")
        )
        ...
        backgroundGroup = QtWidgets.QGroupBox(
            self.context.getText("dialog_new_image", "background")
        )
        backgroundLayout = QtWidgets.QVBoxLayout()
        self.r1 = QtWidgets.QRadioButton(
            self.context.getText("dialog_new_image", "transparent")
        )
        self.r1.setChecked(True)
        self.r2 = QtWidgets.QRadioButton(
            self.context.getText("dialog_new_image", "color")
        )
    ...
    def accept(self):
        ...
        if self.r1.isChecked():
            self.context.newImage(
                self.width.value(), self.height.value(), QtGui.QColor(0, 0, 0, 0)
            )
        else:
...
class ResizeImageDialog(QtWidgets.QDialog):
    ...
    def __init__(self, context, Parent=None):
        ...
        dimensionGroup = QtWidgets.QGroupBox(
            self.context.getText("dialog_resize", "dimension")
        )
        dimensionLayout = QtWidgets.QVBoxLayout()
...
class ResizeCanvasDialog(QtWidgets.QDialog):
    ...
    def __init__(self, context, Parent=None):
        ...
        dimensionGroup = QtWidgets.QGroupBox(
            self.context.getText("dialog_resize_canvas", "dimension")
        )
        dimensionLayout = QtWidgets.QVBoxLayout()
...
class Preferences(QtWidgets.QDialog):
    ...
    def __init__(self, context, signals, Parent=None):
        ...
        self.preferences.addItem(
            self.context.getText("dialog_preferences", "item_language")
        )
        self.preferences.addItem(
            self.context.getText("dialog_preferences", "item_theme")
        )
        self.preferences.addItem(
            self.context.getText("dialog_preferences", "item_matrix_grid")
        )
    ...
    def createLanguageView(self):
        ...
        g = QtWidgets.QGroupBox(
            self.context.getText("dialog_preferences", "item_language_language")
        )
    ...
    def createUICustomizationView(self):
        ...
        hbox.addWidget(
            QtWidgets.QLabel(
                self.context.getText("dialog_preferences", "item_theme_theme")
            )
        )
    ...
    def createMatrixGridView(self):
        ...
        g = QtWidgets.QGroupBox(
            self.context.getText("dialog_preferences", "item_matrix_grid_dimension")
        )
        ...
        if self.langCodes[self.language.currentIndex()] != self.context.lang:
            QtWidgets.QMessageBox.information(
                ...
                self.context.getText(
                    "dialog_preferences", "item_language_changed_title"
                ),
                self.context.getText(
                    "dialog_preferences", "item_language_changed_message"
                ),
            )
        self.context.setDefault(
            "language", "lang", self.langCodes[self.language.currentIndex()]
        )

        self.context.matrixGridWidth = self.matrixGridWidth.value()
        self.context.setDefault(
            "grid", "matrix_grid_width", self.context.matrixGridWidth
        )
        self.context.matrixGridHeight = self.matrixGridHeight.value()
        self.context.setDefault(
            "grid", "matrix_grid_height", self.context.matrixGridHeight
        )

        if (
            self.context.getDefault("theme", "theme", "aquamarine")
            != self.themeDirs[self.theme.currentIndex()]
        ):
            QtWidgets.QMessageBox.information(
                self,
                self.context.getText("dialog_preferences", "item_theme_changed_title"),
                self.context.getText(
                    "dialog_preferences", "item_theme_changed_message"
                ),
            )
        self.context.setDefault(
            "theme", "theme", self.themeDirs[self.theme.currentIndex()]
        )
