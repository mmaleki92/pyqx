...
class ScrollArea(QtWidgets.QScrollArea):
            ...
            if self.canvas.width() < w:
                self.canvas.move(
                    int(
                        (
                            w
                            - self.context.getImagePos(self.index).image.width()
                            * self.context.getImagePos(self.index).zoom
                        )
                        / 2
                    ),
                    int(self.canvas.y()),
            ...
            if self.canvas.height() < h:
                self.canvas.move(
                    ...
                    int(
                        (
                            h
                            - self.context.getImagePos(self.index).image.height()
                            * self.context.getImagePos(self.index).zoom
                        )
                        / 2
                    ),
    ...
    def setNewImageZoom(self):
        ...
        self.context.images[-1].zoom = int(
            min(
                w / self.context.images[-1].image.width(),
                h / self.context.images[-1].image.height(),
            )
        )
