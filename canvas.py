...
class Canvas(QtWidgets.QLabel):
    ...
    def enterEvent(
        self, event
    ):  # Cuando entra el ratón en el Canvas cambiamos el cursor
    ...
    def changeCursor(self):
        ...
        cursors = [
            0,
            0,
            self.context.pencilCur,
            self.context.eraserCur,
            self.context.colorPickerCur,
            self.context.fillCur,
            0,
            0,
        ]
    ...
    def mousePressEvent(self, event):
            ...
            if event.button() == Qt.LeftButton:
                if not self.image().selection:
                    ...
                    if self.image().selection.rect.contains(QtCore.QPoint(x, y)):
                        ...
                        self.image().selectionGrabPoint = QtCore.QPoint(
                            x - self.image().selection.rect.x(),
        ...
        # Lápiz
            ...
            if self.drawing:
                ...
                color = (
                    self.context.primaryColor
                    if event.button() == Qt.LeftButton
                    else self.context.secondaryColor
                )
                size = self.context.pencilSize
                if (
                    event.button() == Qt.RightButton
                    and self.context.secondaryColorEraser
                ):
        ...
        # Goma
            ...
            if event.button() == Qt.LeftButton or event.button() == Qt.RightButton:
                self.lastPoint = QtCore.QPoint(x, y)
                self.image().paintPoint(
                    x, y, self.image().bgColor, self.context.eraserSize
                )
                self.signals.updateCanvas.emit()
                self.drawing = True
        ...
        # Cubo
            ...
            if event.button() == Qt.LeftButton:
                self.fillImageFaster(
                    (x, y),
                    self.context.primaryColor,
                    self.context.currentQImage().pixel(x, y),
                    self.context.currentQImage(),
                )
            elif event.button() == Qt.RightButton:
                self.fillImage(
                    (x, y),
                    self.context.secondaryColor,
                    self.context.currentQImage().pixel(x, y),
                    self.context.currentQImage(),
                )
    ...
    def mouseMoveEvent(self, event):
        ...
        # Goma
            ...
            if event.buttons() == Qt.LeftButton or event.buttons() == Qt.RightButton:
                endPoint = QtCore.QPoint(x, y)
                self.drawLineTo(
                    QtCore.QPoint(x, y), self.image().bgColor, self.context.eraserSize
                )
                self.signals.updateCanvas.emit()
                self.lastPoint = QtCore.QPoint(endPoint)
        ...
        if event.buttons() == Qt.MiddleButton:
			...
			"""
            sb.setValue(
                self.mapToParent(self.pos()).x()
                * sb.maximum()
                / float(-self.width() + self.parent.width())
            )
            ...
            for j in range(size):
                if m[i][j]:
                    ...
                    if j == size - 1 or not m[i][j + 1]:
                        x0 = int(
                            (x + size / 2 + (j - size) + 2) * self.image().zoom - 1
                        )
                        y0 = int((y - size / 2 + i) * self.image().zoom)
                        y1 = int((y - size / 2 + i + 1) * self.image().zoom - 1)
                        painter.drawLine(x0, y0, x0, y1)
                    ...
                    if i == size - 1 or not m[i + 1][j]:
                        ...
                        y0 = int(
                            (y + size / 2 + (i - size) + 2) * self.image().zoom - 1
                        )
                        painter.drawLine(x0, y0, x1, y0)
    ...
    def mouseReleaseEvent(self, event):
        ...
        # Selección
        if (
            self.context.currentTool == Pixeler.Tools.Selection
            and event.button() == QtCore.Qt.LeftButton
        ):

            if self.selecting:
                print(
                    "Selection made starting at ("
                    + str(self.image().selection.origin.x())
                    + ", "
                    + str(self.image().selection.origin.y())
                    + ") and ending at ("
                    + str(x)
                    + ", "
                    + str(y)
                ...
                self.image().selection.originTopLeft = QtCore.QPoint(
                    self.image().selection.rect.x(), self.image().selection.rect.y()
                )
        ...
        # Degradado
        if (
            self.context.currentTool == Pixeler.Tools.Gradient
            and event.button() == QtCore.Qt.LeftButton
        ):
            ...
            if self.selecting:
                ...
                x1, y1 = (
                    self.context.gradient.origin.x(),
                    self.context.gradient.origin.y(),
                )
                x2, y2 = x, y
                ...
                self.context.gradient.originTopLeft = QtCore.QPoint(
                    self.context.gradient.rect.x(), self.context.gradient.rect.y()
                )
    ...
    def paintEvent(self, event):
        ...
        if self.image().bgColor == QtGui.QColor(0, 0, 0, 0):
            painter.fillRect(
                self.rect(), QtGui.QBrush(QtGui.QImage("images/transparent.png"))
            )
        ...
        if (
            not self.selecting
            and self.image().selection != None
            and self.image().selection.finished
            and self.image().selection.image != None
        ):
            rect = QtCore.QRect(
                self.image().selection.rect.topLeft() * self.image().zoom,
        ...
        if (
            self.context.currentTool == Pixeler.Tools.Pencil
            or self.context.currentTool == Pixeler.Tools.Eraser
        ):
            # Draw ToolHint
    ...
    def zoom(self):  # Cosas que hacer cuando se aplica un zoom
        ...
        if (
            self.image().selection != None
        ):  # Calcular la nueva geometría de la selección, en caso que haya
            self.calcNewSelectionGeometry()
        self.resize()

    def resize(self):
        print(self.image().zoom)
        super(Canvas, self).resize(
            QtCore.QSize(
                self.image().image.width() * self.image().zoom,
    ...
    def applySelection(self):
        ...
        if self.image().selection != None:
            ...
            if (
                self.image().selection.originTopLeft
                != self.image().selection.rect.topLeft()
            ):
                self.image().addHistoryStep()
            self.signals.updateCanvas.emit()
    ...
    def cutImage(self):
        ...
        if (
            self.context.imagePos == self.index
        ):  # Cortar sólo si este Canvas es el actual
    ...
    def copyImage(self):
        ...
        if (
            self.context.imagePos == self.index
        ):  # Copiar sólo si este Canvas es el actual
            clipboard = QtWidgets.QApplication.clipboard()
            if self.image().selection != None:
                clipboard.setImage(self.image().selection.image)
    ...
    def pasteImage(self):
        ...
        if (
            self.context.imagePos == self.index
        ):  # Pegar sólo si este Canvas es el actual
            print("Pasting image")
    ...
    def clearImage(self):
        ...
        if (
            self.context.imagePos == self.index
        ):  # Eliminar sólo si este Canvas es el actual
    ...
    def fillImageFaster(self, begin, paint, current, image):
        ...
        self.image().image = QtGui.QImage(
            pil_im.convert("RGB").tobytes("raw", "RGB"),
            pil_im.size[0],
            pil_im.size[1],
    ...
    def calcNewSelectionGeometry(self):
        ...
        if self.image().selection != None and self.image().selection.finished:
            self.image().selection.setGeometry(
                self.image().selection.rect.x(),
